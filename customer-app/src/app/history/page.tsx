  ...
  return (
    <main className="mx-auto min-h-screen max-w-md bg-[#FFF9EA] pb-24 text-[#211715] shadow-2xl shadow-black/10">
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/45 p-5 backdrop-blur-[3px]" onClick={() => setSelectedOrder(null)}>
          <section className="w-full max-w-[340px] rounded-[18px] bg-[#FFFDF8] px-5 pb-5 pt-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <button
              className="mt-4 h-12 w-full rounded-xl bg-[#F34208] text-sm font-black text-white shadow-lg shadow-[#F34208]/20 transition active:scale-[0.99]"
      ...
      <nav className="fixed bottom-0 left-1/2 z-20 grid h-[76px] w-full max-w-md -translate-x-1/2 grid-cols-4 items-center gap-1 border-t border-[#F0D7B0] bg-white px-3 pb-2 pt-2 shadow-2xl shadow-black/10">
        ...
        <Link className="text-center text-[11px] font-bold text-[#F34208]" href="/history">
          <HistoryIcon />
          <span>Lịch sử</span>
        </Link>
        <Link className="text-center text-[11px] font-semibold text-[#6F625C]" href="/account">
          <UserIcon />
