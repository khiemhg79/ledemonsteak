...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!token) {
    return NextResponse.json({ error: "Bạn cần đăng nhập bằng tài khoản nhân viên." }, { status: 401, headers: corsHeaders() })
  }

  try {
    const payload = verifyToken(token)
    if (!["STAFF", "ADMIN"].includes(payload.role)) {
      return NextResponse.json({ error: "Tài khoản không có quyền tạo mã QR." }, { status: 403, headers: corsHeaders() })
    }
  } catch {
  ...
  return NextResponse.json({ url: `${base}?tableId=${table.id}`, tableId: table.id }, { headers: corsHeaders() })
}
