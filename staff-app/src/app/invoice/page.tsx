...
export default function InvoicePage() {
  ...
  async function loadOrders() { setError(""); try { const [orderList, invoiceList] = await Promise.all([apiGet("/api/orders?view=staff"), apiGet("/api/invoices")]); setOrders(orderList); setInvoices(invoiceList) } catch (err) { setError(err instanceof Error ? err.message : "Không tải được hóa đơn.") } }
